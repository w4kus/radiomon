...
constexpr size_t tapNum = sizeof(lp_test_6k_48k_large) / sizeof(lp_test_6k_48k_large[0]);

template<typename T>
typename grfirinterp<T>::sptr grfirinterp<T>::make(short interpolate, bool adjustGain)
{
    return gnuradio::make_block_sptr<grfirinterp_impl<T>>(interpolate, adjustGain);
}
 ...
 */
template<typename T>
grfirinterp_impl<T>::grfirinterp_impl(short interpolate, bool adjustGain)
    : gr::sync_interpolator(
          ...
          d_adjustGain { adjustGain }
{
}

template<>
grfirinterp_impl<float>::grfirinterp_impl(short interpolate, bool adjustGain)
    : gr::sync_interpolator(
          "grfirinterp",
          gr::io_signature::make(
          ...
          d_adjustGain { adjustGain }
{
    d_InterpF = std::make_unique<dsp::firinterp_ff>(interpolate, 
                util::make_aligned_ptr<float>(tapNum, lp_test_6k_48k_large), adjustGain);
}

template<>
grfirinterp_impl<gr_complex>::grfirinterp_impl(short interpolate, bool adjustGain)
    : gr::sync_interpolator(
          "grfirinterp",
          gr::io_signature::make(
          ...
          d_adjustGain { adjustGain }
{
    d_InterpC = std::make_unique<dsp::firinterp_cc>(interpolate, 
                util::make_aligned_ptr<float>(tapNum, lp_test_6k_48k_large), adjustGain);
}
 ...
 */
template<typename T>
grfirinterp_impl<T>::~grfirinterp_impl() {}

template<typename T>
int grfirinterp_impl<T>::work(int noutput_items,
...
template<>
int grfirinterp_impl<float>::work(int noutput_items,
                           gr_vector_const_void_star& input_items,
                           gr_vector_void_star& output_items)
{
    ...
    auto apIn = util::make_aligned_ptr<float>(noutput_items / d_interpolate, in);
    util::aligned_ptr<float> apOut { };

    d_InterpF->interp(apIn, apOut);
...
template<>
int grfirinterp_impl<gr_complex>::work(int noutput_items,
                           gr_vector_const_void_star& input_items,
                           gr_vector_void_star& output_items)
{
    ...
    auto apIn = util::make_aligned_ptr<gr_complex>(noutput_items / d_interpolate, in);
    util::aligned_ptr<gr_complex> apOut { };

    d_InterpC->interp(apIn, apOut);
...
template class grfirinterp<float>;
template class grfirinterp<gr_complex>;
